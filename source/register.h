...
class Register : public QObject, public BitSet {
 ...
 public:
  ...
  /* zero constructor*/
  explicit Register(const QString &name = QString(), bool is_sub = false,
                    const QByteArray &init_json = QByteArray(),
                    quint32 options = AllowSameName);
  ...
  /* copies register src to dst .name not copy. success when dst.size == 0 and
   * register dst same as src*/
  ...
  /*
      (***)
          ...
          as former register.
          Tchnical particularity of register class instance is that sub register
     does not delete bits on destruction. Otherwise it is normal register. Bits
     which are grouped to sub register will be referenced twice. Example:
  ...
  */
  ...
  static Register *makeSubRegister(Register *parent,
  ...
  const QString toString(const QString &format = "@name=@value;",
                         bool skip_empty = true);
  bool fromString(const QString &text, const char ln_separator = ';',
...
////        :Register(){ setSub(1);  }
////// [not neccessary]
//////    SubRegister(const QString &init_script,const QString  &name =
///QString())
//////        :Register(init_script,name){setSub(1);}
//////    SubRegister(qint32 bit_count,const QString  &name = QString(), quint32
...
////{
////    d << QString("Register(name=%1,
///bits=%2)").arg(reg.name()).arg(reg.size()); /    return d;
////}
