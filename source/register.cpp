 ...
 * @description:
 ****************************************************************************/
Register::Register(const QString &name, bool is_sub,
                   const QByteArray &init_json, quint32 options)
    : QObject(), BitSet(name, -1, !is_sub) {
  m_is_sub = is_sub;
  m_offset = 0;
  mp_temporary = NULL;
  ...
  if (!init_json.isEmpty()) {
    this->blockSignals(1);
    loadJsonData(init_json, options);
    this->blockSignals(0);
  }
}
 ...
 ****************************************************************************/
Register::Register(Register &reg) : QObject(), BitSet() {
  m_is_sub = false;
  m_offset = 0;
  mp_temporary = NULL;
  m_update_policy = UpdateOnChange;
...
void Register::clear() {
  if (!isSub()) {
    foreach (BitField *pfield, m_fields) { delete pfield; }
  }
  BitSet::clear();
  m_fields.clear();
  m_offset = 0;
...
bool Register::loadJsonData(const QByteArray &json_data, quint32 options) {
  ...
  if (parseError.error != QJsonParseError::NoError) {
    WARNING(QString("Parse error at %1:%2")
                .arg(parseError.offset)
                .arg(parseError.errorString()));
    qDebug() << QString("Parse error at %1:%2")
                    .arg(parseError.offset)
                    .arg(parseError.errorString());
    return false;
  }
  ...
  for (int i = 0; i < fields_array.count(); i++) {
    ...
    if (available_keys.contains("offset")) {
      ...
      if (field["offset"].isString())
        ...
        offset = (quint32)field["offset"].toInt();
    ...
    if (f != 0) {
      f->setValue(parser.value_u32);
      f->setConstant(parser.value_readonly);

      if (options & AbsoluteRange && parser.has_range == 2) {
        int lsb = parser.range_lsb + m_offset;
        if (size() < lsb + 1) {
          resize(lsb);
        }
      }
 ...
 *              m_chain is a list container of single bits. each bit is
 *represented by pointer to object Bit. So m_chain index equals to bit position
 *in list. Bit are stored from LSB to msb. Example: Register("A[8];B[8]") in
 *m_chain is A(0,,,7),B(0,,,7),... name is the same. There are two types of
 *parser used I) has format "@register=NAME;@bit=A[8] @value=123
 *@descr=DESCRIPTION;..." II) second format "A[8]=0;B[8]=123#DESCRIPTION;,,,"
...
bool Register::addField(const QString &fieldname, quint32 options,
  ...
  if (!fieldname.isEmpty()) {
    ...
    if (options & AllowSameName) {
      if (contains(parser.captured.name)) {
        field(parser.captured.name)->join(f);
        delete f;
        f = 0;
      }
    }
...
int Register::fieldsCount() const { return m_fields.count(); }
 ...
 ****************************************************************************/
bool Register::contains(const QString &name) { return field(name) != 0; }
...
const QString Register::toString(const QString &format, bool skip_empty) {
  ...
  for (int i = 0; i < size(); i++) {
    ...
    if (pfield) {
      if (pfield->size() > 1)
        dict["name"] =
            QString("%1[%2]").arg(pfield->name()).arg(pfield->size());
      else
      ...
      if (!skip_empty) {
        ...
        dict["range"] =
            QString("%2:%1").arg(bak_i).arg(bak_i + _undef_size - 1);
        replaceTagsInLine(&line, dict);
        result += line;
      }
    }
...
bool Register::fromString(const QString &text, const char ln_separator,
                          const char eq_separator) {
  bool ok = false;
...
BitField *Register::field(int index) { return m_fields.at(index); }
...
Register *Register::makeSubRegister(Register *parent, const QString &name) {
  Register *preg = new Register(name, AllowSameName | SubRegister);
  preg->setParentRegister(parent);
  return preg;
}
 ...
 ****************************************************************************/
quint32 Register::fieldValue(
    const QString &field)  // ------------------------------------remake
{
  quint32 result = 0;
  const QRegExp rx_fullname("(.*)\\[(.*)\\]");
  QString regname;
      ...
      switch (i) {
        ...
        case 2:
          if (range.isEmpty()) {
            ...
            if (!preg->isEmpty()) result = preg->toUInt();
          } else
            fail = true;
          break;
      }
...
Register *Register::sub(const QString &extra_name,
                        const QVariant &extra_value) {
  if (mp_temporary == NULL) {
    makeTemporary();
  }
  mp_temporary->clear();
...
QStringList Register::extras() const { return m_extra.keys(); }
...
Register &Register::operator[](const char *field) { return *sub(field); }

Register &Register::operator[](const QString &field) { return *sub(field); }

void Register::makeTemporary() {
  mp_temporary = Register::makeSubRegister(this, "sub_" + name());
}

void Register::clearTemporary() { mp_temporary->clear(); }
...
quint32 Register::crc(int bits, quint32 seed, quint32 poly, bool padding,
                      QString *ptext) {
  Register r_poly(QString("payload[%d]").arg(bits + 1));
  r_poly.setUInt(poly);
  int sz = r_poly.size();
  quint32 crc = 0;
  Register reg(*this);

  // append seed to the end because we assume that we calculate from MSB but
  // register starts from LSB
  reg.addField(QString("seed[%1]=%2").arg(bits).arg(seed), 0);

  // insert bits count according to CRC size 8,16,32...
  if (padding) reg.addField(QString("padding[%1]=0").arg(bits), 0);

  // only for debugging purposes
  QString space;
  if (ptext) {
    ptext->append(reg.toBitString(Register::MSB) + " | " +
                  r_poly.toBitString(Register::MSB) + "\n");
  }
    ...
    if (reg.bitValue(reg.size() - i - 1) != 0) {
      ...
      if (ptext) {
        ptext->append(QString("\n%1%2").arg(space).arg(
            QString(r_poly.toBitString(Register::MSB))));
        ptext->append("\n----------------");
